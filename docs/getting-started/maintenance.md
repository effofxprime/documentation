...
That will turn new full node into validator. This process is important part of providing infrastructure services for Beezee chain.
...
**If you want to evaluate presented process use Beezee Testnet, which is kept running for evaluation, testing and learning purposes.**
:::
...
#### Stop and disable Beezee service
In this step we will stop and disable existing Beezee service.
```bash
sudo systemctl stop bze.service
sudo systemctl disable bze.service
```

To make sure that service has been stopped you can check if any new logs are generated by service
```bash
sudo journalctl -u bze -f
```
...
```bash
mv ${HOME}/.bze/config/node_key.json ${HOME}/.bze/config/node_key.json.validator
mv ${HOME}/.bze/config/priv_validator_key.json ${HOME}/.bze/config/priv_validator_key.json.validator
```
...
```bash
scp ${HOME}/.bze/config/node_key.json.validator ADMINUSER@your.new.server.ip:/home/bze/.bze/config
scp ${HOME}/.bze/config/priv_validator_key.json.validator ADMINUSER@your.new.server.ip:/home/bze/.bze/config
```
**NOTE TO CHANGE SCP TO RSYNC SO THAT FILE PERMISSIONS CAN BE SET PROPERLY AFTER A COPY USING THE CORRECT USER ACCOUNTS TO ACCESS THE SERVER**

### PART 2: Steps to perform on new server

::: tip NOTE:
Assumption is that new system, which will assume validator role is already configured as Beezee full node and fully synchronized.
:::
...
```bash
mv ${HOME}/.bze/config/node_key.json ${HOME}/.bze/config/node_key.json.node
mv ${HOME}/.bze/config/priv_validator_key.json ${HOME}/.bze/config/priv_validator_key.json.node
```
...
This step assumes that keys copied from original validator are located in Beezee node config folder.
```bash
cp ${HOME}/.bze/config/node_key.json.validator ${HOME}/.bze/config/node_key.json
cp ${HOME}/.bze/config/priv_validator_key.json.validator ${HOME}/.bze/config/priv_validator_key.json
```
...
```bash
sudo systemctl enable bze.service
...
First, as your admin user, stop the bze.service

```bash
sudo systemctl stop bze
```

It is best to start from scratch.
Continue as your `bze` user.
...
rpc_servers = "https://trpc.rpc.erialos.me:443,https://mainnet-rpc.getbze.com:443"
...
```bash
nano /home/bze/.bze/config/app.toml
```
...
```bash
systemctl start bze && journalctl -u bze.service -f
```
...
Like all beezee nodes, you can follow the instructions already provided to get started!
...
Now start your beezee daemon, watch the logs, and allow your new sentry to first catch up and download the blockchain.  Once it has caught up, proceed with the next steps.
...
```bash
systemctl restart bze && journalctl -u bze -f
```